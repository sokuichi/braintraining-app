      ...
      function renderProgress() {
        ...
        Object.keys(moduleInfo).forEach((key) => {
          const module = state.modules[key];
          const item = document.createElement("li");
          item.innerHTML = `<strong>${escapeHtml(moduleInfo[key].label)}</strong><span>d${module.difficulty}, ${module.xp} XP, ${module.sessions} sessions, best ${module.best}%. ${escapeHtml(module.lastNote)}</span>`;
      ...
      function terminalHelp() {
        ...
        terminalWriteBlock("Terminal controls", [
          ...
          "faq, security, privacy, research, export, backup, import backup, wipe confirm, sound"
        ]);
      }
      ...
      function securityLines() {
        return [
          ...
          "Backup and import use sanitized JSON. Prototype-pollution keys are ignored, numbers are clamped, and text fields are length-limited.",
          "Visible session history is escaped before rendering, so tampered local data is treated as text instead of markup.",
          "Use backup before wipe confirm. Wipe confirm removes only this app's local progress key.",
          `localStorage: ${storageStatus()} | saved payload: ${Math.round(JSON.stringify(state).length / 1024)} KB`
        ];
      }
