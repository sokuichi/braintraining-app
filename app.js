      ...
      function sanitizeState(nextState) {
        ...
        nextState.history = Array.isArray(nextState.history)
          ? nextState.history.filter((entry) => entry && moduleInfo[entry.module]).slice(0, 24)
          : [];
        return nextState;
      }
      ...
      function sanitizeModule(key, module) {
        ...
        merged.recent = Array.isArray(merged.recent)
          ...
          : [];
        merged.lastNote = typeof merged.lastNote === "string" ? merged.lastNote : "Ready";
        if (key === "systems") {
      ...
      function animateVfx() {
        ...
        ctx.globalAlpha = 1;
        window.requestAnimationFrame(animateVfx);
      }
      ...
      function generateQuestion() {
        ...
        const advanced = [makeCaseReasoningQuestion, makeEvidenceWeighingQuestion, makeConstraintOptimizationQuestion, makeArgumentFlawQuestion, makeBayesianQuestion, makeCausalGraphQuestion, makeProofChainQuestion, makeScheduleOptimizerQuestion];
      ...
      function generateSystemsProblem() {
        ...
        const generators = {
          ...
          mixed: [makeCausalGraphQuestion, makeBayesianQuestion, makeProofChainQuestion, makeScheduleOptimizerQuestion, makeConstraintOptimizationQuestion]
        };
      ...
      function initializeTerminal() {
        ...
        terminalWrite("Type help, stats, program, quests, modules, hard, train memory, start focus, set questions 8, or boost all 2.", "system");
      ...
      function moduleSummaryLine(key) {
        const module = state.modules[key];
        return `${moduleInfo[key].code} ${moduleInfo[key].label}: stored d${module.difficulty}, effective d${moduleDifficulty(key)}, ${module.xp} XP, ${module.sessions} sessions, best ${module.best}%, ${module.lastNote}`;
      }
      ...
      function bindEvents() {
        ...
        on($("#terminalApplyCustomize"), "click", () => {
          ...
          saveState();
          renderAll();
          terminalWrite(`Customizer applied: ${moduleInfo[key].label} d${difficulty}, theme ${state.settings.theme}, vfx ${["off", "low", "arcade", "max"][state.settings.vfx]}.`, "success");
          playSfx("complete");
