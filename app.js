      ...
      const memoryModes = {
        ...
        manipulation: {
          ...
          description: "Compare the manipulated stimulus, not only the raw one."
        },
        quad: {
          label: "Quad",
          grid: 5,
      ...
      function moduleDifficulty(key) {
        ...
        return clamp(base + hardBonus, 1, 12);
      }
      ...
      function memoryUnlockForDifficulty(difficulty) {
        if (difficulty >= 7) return "quad";
        if (difficulty >= 4) return "manipulation";
        if (difficulty >= 2) return "multi";
        return "dual";
      }

      function modeUnlocked(mode) {
        const order = ["dual", "multi", "manipulation", "quad"];
        const current = order.indexOf(memoryUnlockForDifficulty(moduleDifficulty("memory")));
        return order.indexOf(mode) <= current;
      }
        ...
        Object.keys(moduleInfo).forEach((key) => {
          ...
          card.innerHTML = `
            ...
            <div class="module-meta">
              ...
              <span>d${moduleDifficulty(key)} - ${difficultyNote(key)}</span>
            </div>
            <h3>${info.label}</h3>
      ...
      function hitTarget(event) {
        ...
        playSfx(aimRound.streak > 0 && aimRound.streak % 5 === 0 ? "combo" : "hit");
        updateAimHud();
        moveTarget();
      }
      ...
      function generateQuestion() {
        ...
        const advanced = [makeCaseReasoningQuestion, makeEvidenceWeighingQuestion, makeConstraintOptimizationQuestion, makeArgumentFlawQuestion];
      ...
      function renderMemoryStats() {
        ...
        const unlocks = [
          { mode: "multi", at: 2 },
          { mode: "manipulation", at: 4 },
          { mode: "quad", at: 7 }
        ];
        const next = unlocks.find((item) => moduleDifficulty("memory") < item.at);
        $("#memoryUnlock").textContent = next ? `${memoryModes[next.mode].label} d${next.at}` : "max modes";
        updateMemoryModeButtons();
      }
      ...
      function setMemoryMode(mode) {
        ...
        if (!modeUnlocked(mode)) {
          $("#memoryFeedback").textContent = `${memoryModes[mode].label} unlocks at higher Memory difficulty.`;
          playSfx("wrong");
          return;
        }
      ...
      function updateMemoryModeButtons() {
        $$("[data-memory-mode]").forEach((button) => {
          ...
          button.disabled = !unlocked || memory.active;
          button.classList.toggle("is-active", button.dataset.memoryMode === memoryConfig().mode);
          button.textContent = unlocked ? memoryModes[button.dataset.memoryMode].label : `${memoryModes[button.dataset.memoryMode].label} locked`;
        });
      ...
      function renderCalibrationAnswers() {
        ...
        grid.innerHTML = "";
        calibration.current.options.forEach((option) => {
          const button = document.createElement("button");
          button.className = "answer-btn";
          button.type = "button";
          button.textContent = option;
      ...
      function answerCalibration(button, option) {
        ...
        const correct = option === calibration.current.answer;
        ...
        $$("#calibrationAnswerGrid .answer-btn").forEach((item) => {
          item.disabled = true;
          if (item.textContent === calibration.current.answer) item.classList.add("is-correct");
        });
        if (!correct) button.classList.add("is-wrong");
      ...
      function initializeTerminal() {
        ...
        terminalWrite("Type help, stats, hard, train memory, set questions 8, research, export, or sound.", "system");
      }
      ...
      function terminalHelp() {
        terminalWrite("Commands:", "system");
        terminalWrite("help - list commands");
        terminalWrite("stats - show XP, level, mastery, and module difficulties");
        terminalWrite("train <module> - open aim/questions/memory/focus/creativity/analogy/spatial/calibration");
        terminalWrite("set <module> <1-10> - set stored module difficulty");
        terminalWrite("hard - hard mode on, depth extreme, session load heavy");
        terminalWrite("normal - hard mode off, depth deep, session load standard");
        terminalWrite("research - open the evidence section");
        terminalWrite("export - copy progress report to clipboard");
        terminalWrite("sound - play the level SFX");
        terminalWrite("clear - clear terminal output");
      }
      ...
      function runTerminalCommand(raw) {
        ...
        if (!command) return;
        terminalWrite(`> ${command}`, "command");
        const [verb, second, third] = command.toLowerCase().split(/\s+/);
        if (verb === "help") {
          terminalHelp();
          return;
        }
        ...
        if (verb === "stats") {
          ...
          terminalWrite(`XP ${state.xp} | level ${levelFromXp(state.xp)} | streak ${state.streak} | mastery ${progress.mastery}% | rank ${progress.rank}`, "system");
          Object.keys(moduleInfo).forEach((key) => {
            const module = state.modules[key];
            terminalWrite(`${moduleInfo[key].label}: stored d${module.difficulty}, effective d${moduleDifficulty(key)}, ${module.xp} XP, best ${module.best}%`);
          });
          return;
        }
        if (verb === "hard") {
        ...
        if (verb === "research") {
          setView("research");
          terminalWrite("Opened research base.", "system");
          return;
        }
        if (verb === "export") {
          copyProgressReport();
