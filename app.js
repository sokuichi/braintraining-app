      ...
      const IDENTITY_VERSION = "geist-rint-v2-perf";
      const TARGET_VISUAL_FPS = 360;
      ...
      const pointerState = {
        ...
        followX: window.innerWidth / 2,
        followY: window.innerHeight / 2,
        previousTargetX: window.innerWidth / 2,
        previousTargetY: window.innerHeight / 2,
        visible: false,
        frame: 0,
        mode: "",
        cursor: null,
      ...
      let rintInstructionsPrompted = false;
      let rintInstructionsReturnFocus = null;
      ...
      function loadState() {
        ...
        if (saved?.settings?.identityVersion !== IDENTITY_VERSION) {
          loaded.settings.guiMode = "simple";
          loaded.settings.vfx = Math.min(loaded.settings.vfx, 1);
          loaded.settings.identityVersion = IDENTITY_VERSION;
        }
      ...
      function showRintInstructions() {
        const modal = $("#rintInstructionsModal");
        if (!modal || !$("#rintInstructionsClose")) return;
        rintInstructionsReturnFocus = document.activeElement instanceof HTMLElement ? document.activeElement : null;
        modal.hidden = false;
        document.body.classList.add("modal-open");
        window.setTimeout(() => $("#rintInstructionsClose").focus(), 0);
      }

      function closeRintInstructions() {
        const modal = $("#rintInstructionsModal");
        if (!modal) return;
        modal.hidden = true;
        document.body.classList.remove("modal-open");
        if (rintInstructionsReturnFocus && document.contains(rintInstructionsReturnFocus)) {
          rintInstructionsReturnFocus.focus();
        }
        rintInstructionsReturnFocus = null;
      }

      function promptRintInstructions() {
        if (rintInstructionsPrompted || currentView !== "rint") return;
        rintInstructionsPrompted = true;
        showRintInstructions();
      }
      ...
      function on(target, event, handler, label = event, options) {
        if (!target) {
          console.warn(`Missing event target for ${label}`);
          return;
        }
        target.addEventListener(event, safe(label, handler), options);
      }
      ...
      function triggerVfx(type, origin = {}) {
        ...
        const count = Math.round(countBase * Math.min(intensity, 2) * 0.38);
      ...
      function burstVfx(x, y, count, colorName, type) {
        ...
        vfx.particles = vfx.particles.slice(-180);
      }
      ...
      function flashVfx(type) {
        ...
        window.requestAnimationFrame(() => flash.classList.add("is-live"));
      }

      function shakeScreen() {
        document.body.classList.remove("screen-shake");
        window.requestAnimationFrame(() => document.body.classList.add("screen-shake"));
      }

      function animateVfx(time = 0) {
        if (!vfx.running || !vfx.ctx) return;
        const now = time || performance.now();
        const deltaScale = vfx.lastTime ? clamp((now - vfx.lastTime) / BASE_FRAME_MS, 0.25, 2) : 1;
        vfx.lastTime = now;
        const ctx = vfx.ctx;
        ctx.clearRect(0, 0, window.innerWidth, window.innerHeight);
        vfx.particles = vfx.particles.filter((particle) => {
          particle.age += deltaScale;
          ...
          particle.vy += 0.08 * deltaScale;
          const drag = Math.pow(0.985, deltaScale);
          particle.vx *= drag;
          particle.vy *= drag;
      ...
      function setPointerFromEvent(event) {
        ...
        const latest = samples && samples.length ? samples[samples.length - 1] : event;
        if (!Number.isFinite(latest.clientX) || !Number.isFinite(latest.clientY)) return;
        pointerState.previousTargetX = pointerState.targetX;
        pointerState.previousTargetY = pointerState.targetY;
        pointerState.targetX = latest.clientX;
        pointerState.targetY = latest.clientY;
        pointerState.sourceTarget = event.target;
      ...
      function syncCustomCursor(time = 0) {
        pointerState.frame = 0;
        const cursor = pointerState.cursor || (pointerState.cursor = $("#customCursor"));
        if (!cursor) return;
        const isVisible = pointerState.visible && state.settings.customCursor;
        if (isVisible !== pointerState.lastVisible) {
          cursor.classList.toggle("is-visible", isVisible);
          pointerState.lastVisible = isVisible;
        }
        if (document.body.classList.contains("cursor-ready") !== isVisible) {
          document.body.classList.toggle("cursor-ready", isVisible);
        }
        if (!isVisible) {
        ...
        const now = time || performance.now();
        const deltaScale = pointerState.lastTime ? clamp((now - pointerState.lastTime) / BASE_FRAME_MS, 0.25, 2) : 1;
        pointerState.lastTime = now;

        pointerState.x = pointerState.targetX;
        pointerState.y = pointerState.targetY;
        const dx = pointerState.targetX - pointerState.followX;
        const dy = pointerState.targetY - pointerState.followY;
        const distance = Math.hypot(dx, dy);
        const follow = pointerState.mode === "aim" ? 0.42 : pointerState.mode === "action" ? 0.36 : 0.32;
        if (distance > 120 || state.settings.lowMotion) {
          pointerState.followX = pointerState.targetX;
          pointerState.followY = pointerState.targetY;
        } else {
          const frameFollow = 1 - Math.pow(1 - follow, deltaScale);
          pointerState.followX += dx * frameFollow;
          pointerState.followY += dy * frameFollow;
        }

        const targetDelta = Math.hypot(pointerState.targetX - pointerState.previousTargetX, pointerState.targetY - pointerState.previousTargetY);
        const angle = Math.atan2(dy, dx || 0.001);
        const stretch = state.settings.lowMotion ? 1 : clamp(1 + targetDelta / 180, 1, 1.2);
        const modeTarget = document.elementFromPoint(pointerState.targetX, pointerState.targetY) || pointerState.sourceTarget;
        if (modeTarget !== pointerState.lastTarget) {
          pointerState.lastTarget = modeTarget;
          updateCursorMode(modeTarget);
        }
        cursor.style.transform = `translate3d(${pointerState.x}px, ${pointerState.y}px, 0)`;
        cursor.style.setProperty("--cursor-lag-x", `${pointerState.followX - pointerState.targetX}px`);
        cursor.style.setProperty("--cursor-lag-y", `${pointerState.followY - pointerState.targetY}px`);
        cursor.style.setProperty("--cursor-angle", `${angle}rad`);
        cursor.style.setProperty("--cursor-stretch", stretch.toFixed(3));

        if (pointerState.mode !== pointerState.lastMode) {
          cursor.classList.toggle("is-action", pointerState.mode === "action");
          cursor.classList.toggle("is-aim", pointerState.mode === "aim");
          pointerState.lastMode = pointerState.mode;
        }
        if (Math.abs(pointerState.followX - pointerState.targetX) > 0.12 || Math.abs(pointerState.followY - pointerState.targetY) > 0.12) {
          scheduleCustomCursorSync();
        }
      }

      function getCustomCursor() {
        return pointerState.cursor || (pointerState.cursor = $("#customCursor"));
      }
      ...
      function updateCursorMode(target) {
        ...
        if (aimStage && (target === aimStage || aimStage.contains(target))) pointerState.mode = "aim";
        else if (target.closest(".btn, .answer-btn, .pill-btn, .nav-btn, button, a, select, input, textarea, [role='button'], [data-terminal-command]")) pointerState.mode = "action";
      ...
      function applySettings() {
        ...
        if (!state.settings.customCursor) {
          ...
          getCustomCursor()?.classList.remove("is-visible", "is-action", "is-aim", "is-pressing");
        }
        ...
        document.body.dataset.targetFps = String(TARGET_VISUAL_FPS);
      ...
      function setView(view) {
        ...
        if (!state.settings.lowMotion) window.requestAnimationFrame(() => document.body.classList.add("view-shift"));
        ...
        if (view === "rint" && previousView !== "rint") {
          window.setTimeout(promptRintInstructions, 180);
        }
      ...
      function scheduleTopbarScrollState() {
        if (scheduleTopbarScrollState.frame) return;
        scheduleTopbarScrollState.frame = window.requestAnimationFrame(() => {
          scheduleTopbarScrollState.frame = 0;
          updateTopbarScrollState();
        });
      }
      ...
      function renderAll() {
        ...
        updateTrainingState();
        renderAnswerTimer();
        if (currentView === "dashboard") animateDashboardCounters();
        uiFx.firstRender = false;
      }

      function renderActiveView(view = currentView) {
        if (view === "dashboard") renderDashboard();
        else if (view === "aim") renderAimConfig();
        else if (view === "questions") renderQuizStats();
        else if (view === "memory") {
          renderMemoryStats();
          renderMemoryCard();
        } else if (view === "focus") renderFocusStats();
        else if (view === "creativity") {
          renderCreativeConfig();
          refreshRubricArcs();
        } else if (view === "analogy") {
          renderAnalogyStats();
          refreshRubricArcs();
        } else if (view === "spatial") renderSpatialStats();
        else if (view === "calibration") renderCalibrationStats();
        else if (view === "systems") renderSystemsStats();
        else if (view === "rint") renderRintStats();
        else if (view === "rrt") renderRrtStats();
        else if (view === "terminal") renderTerminalStatus();
      ...
      const rintComposableRelationKeys = {
        spatial: ["left-of", "right-of", "above", "below", "inside", "contains"],
        trait: ["same-shape", "more-stable", "less-stable", "same-class"],
        quant: ["greater-than", "less-than", "difference-plus", "difference-minus"],
        semantic: ["causes", "category-of", "enables", "requires"],
        comparison: ["more-abstract", "more-concrete", "broader", "narrower", "stronger-evidence", "weaker-evidence"],
        temporal: ["before", "after"],
        directional: ["flows-to", "flows-from"]
      };
      ...
      function generateRintTrial(config) {
        ...
        const targetMatch = canMatch && Math.random() < config.matchRate;
        const item = makeRintItem(config, { mode, matchBack: targetMatch ? back : null, avoidBack: targetMatch ? null : back });
        item.taskMode = mode;
        item.backItem = rawBack;
        item.compareItem = back;
        item.shouldMatch = Boolean(back) && rintSignature(item, mode) === rintSignature(back, mode);
        item.backSummary = rintBackSummary(rawBack, back, mode);
        item.currentState = rintStateLabel(item, mode);
        item.isDistractor = !item.shouldMatch && Boolean(back) && isRintNearMiss(item, back, mode);
      ...
      function makeRintItem(config, options = {}) {
        ...
        for (let attempt = 0; attempt < 40; attempt += 1) {
          item = makeGraphRintItem(config, options);
      ...
      function makeBinaryRintItem(config, options = {}) {
        ...
        const relation = source?.binaryRelation && source.binaryTruth === targetTruth
          ? source.binaryRelation
          : sample([
            { key: "greater-than", label: "greater than", inverse: "less than", family: "quant" },
            { key: "before", label: "before", inverse: "after", family: "temporal" },
            { key: "left-of", label: "left of", inverse: "right of", family: "spatial" },
            { key: "more-abstract", label: "more abstract than", inverse: "more concrete than", family: "comparison" }
          ]);
        const claim = targetTruth
          ? `${a} is ${relation.label} ${c}.`
          : `${c} is ${relation.label} ${a}.`;
        const premises = [
          rintPremiseObject(a, b, { key: relation.key, label: relation.label, phrase: (x, y) => `${x} is ${relation.label} ${y}.` }),
          rintPremiseObject(b, c, { key: relation.key, label: relation.label, phrase: (x, y) => `${x} is ${relation.label} ${y}.` })
        ];
        ...
        const item = {
          ...
          derivedRelation,
          claim,
          premise: `${premises.map((entry) => entry.text).join(" ")} Claim: ${claim}`,
          logicMode: "binary",
          nearMissReason: targetTruth ? "" : "claim reverses the derived endpoint order"
        };
        item.derivedSignature = `${item.category}:${item.relationKey}`;
        item.surfaceSignature = rintSurfaceSignature(item);
        return item;
      }
      ...
      function rintComposableRelations(category) {
        const keys = rintComposableRelationKeys[category] || [];
        const relations = rintRelationBank[category] || rintRelationBank.spatial;
        const filtered = relations.filter((item) => keys.includes(item.key));
        return filtered.length ? filtered : relations;
      }

      function chooseRelationForCategory(category, avoidBack, preferNearMiss) {
        const relations = rintComposableRelations(category);
        if (avoidBack && preferNearMiss && avoidBack.category === category) {
          const avoidRelation = relationByKey(category, avoidBack.relationKey);
          const inverse = relations.find((item) => item.key === avoidRelation.inverse);
      ...
      function makeGraphRintItem(config, options = {}) {
        const mode = options.mode || "rint";
        const source = options.matchBack;
        const avoidBack = options.avoidBack;
        const sourceCategory = source && !String(source.category || "").startsWith("binary-") ? source.category : null;
        const category = mode === "normal" && sourceCategory
          ? sourceCategory
          : mode === "type" && sourceCategory
            ? sourceCategory
            : mode === "rint" && sourceCategory
              ? sourceCategory
              : chooseGraphCategory(config, avoidBack, mode);
        const relationDef = chooseGraphRelation(category, source, avoidBack, mode, config.distractors);
        const premiseCount = source && mode === "normal"
          ? Math.max(2, (source.premises || []).length)
          : (config.stage === "mixed" || config.d >= 4 || Math.random() < 0.28 ? 3 : 2);
        const entities = source && mode === "normal"
          ? (source.entities || source.tokens || []).slice(0, premiseCount + 1)
          : rintTokens(config, premiseCount + 1);
        while (entities.length < premiseCount + 1) entities.push(`${sample(["L", "M", "N"])}${Math.floor(Math.random() * 90 + 10)}`);
        const premises = [];
        for (let index = 0; index < premiseCount; index += 1) {
          premises.push(rintPremiseObject(entities[index], entities[index + 1], relationDef));
        }
        const derivedRelation = {
          from: entities[0],
          to: entities[premiseCount],
          relationKey: relationDef.key,
          relationLabel: relationDef.label,
          text: relationDef.phrase(entities[0], entities[premiseCount])
        };
        const item = {
          taskMode: mode,
          logicMode: "graph",
          ...
          relationKey: relationDef.key,
          relationLabel: relationDef.label,
          tokens: entities.slice(),
          entities,
          premises,
          derivedRelation,
          query: `Derive: ${derivedRelation.text}`,
          premise: `${premises.map((entry) => entry.text).join(" ")} Therefore: ${derivedRelation.text}`,
          nearMissReason: avoidBack && config.distractors ? rintNearMissReason(mode, category, relationDef, avoidBack) : ""
        };
        item.derivedSignature = `${item.category}:${item.derivedRelation.relationKey}`;
        item.surfaceSignature = rintSurfaceSignature(item);
        return item;
      }

      function chooseGraphCategory(config, avoidBack, mode) {
        const categories = config.categories.length ? config.categories : rintCategoryKeys();
        if (mode === "type" && avoidBack?.category) {
          const filtered = categories.filter((category) => category !== avoidBack.category);
          return sample(filtered.length ? filtered : categories);
        }
        if (mode === "rint" && avoidBack?.category && config.distractors && Math.random() < 0.74) return avoidBack.category;
        return sample(categories);
      }

      function chooseGraphRelation(category, source, avoidBack, mode, useDistractors) {
        if (source && mode === "normal") return relationByKey(source.category, source.relationKey);
        if (source && mode === "rint") return relationByKey(source.category, source.relationKey);
        if (source && mode === "type") return chooseRelationForCategory(category, null, false);
        return chooseRelationForCategory(category, avoidBack, useDistractors);
      }

      function rintPremiseObject(from, to, relationDef) {
        return {
          from,
          to,
          relationKey: relationDef.key,
          relationLabel: relationDef.label,
          text: relationDef.phrase(from, to)
        };
      }

      function rintNearMissReason(mode, category, relationDef, avoidBack) {
        if (!avoidBack) return "";
        if (mode === "normal") return "shares part of the surface token graph";
        if (mode === "type") return "relation wording feels similar but the category changed";
        if (mode === "binary") return "claim truth flips while the chain looks familiar";
        if (category === avoidBack.category && relationDef.key !== avoidBack.relationKey) return "same relation family, different derived relation";
        return "near-miss relation graph";
      }
      ...
      function rintSignature(item, mode) {
        if (!item) return "";
        if (mode === "normal") return item.surfaceSignature || rintSurfaceSignature(item);
        if (mode === "type") return item.category;
        if (mode === "binary") return String(Boolean(item.binaryTruth));
        return item.derivedSignature || `${item.category}:${item.relationKey}`;
      }

      function rintSurfaceSignature(item) {
        if (!item) return "";
        const tokens = (item.entities || item.tokens || []).join("|");
        const relations = (item.premises || []).map((entry) => entry.relationKey).join(">");
        return `${tokens}:${relations}`;
      }

      function rintStateLabel(item, mode) {
        if (!item) return "none";
        if (mode === "normal") return `surface graph ${rintEntityLabel(item)}`;
        ...
        return `${rintCleanCategory(item.category)} / derive ${rintDerivedText(item)}`;
      }
      ...
      function isRintNearMiss(item, back, mode) {
        ...
        if (mode === "normal") return item.tokens.some((token) => back.tokens.includes(token)) || item.relationKey === back.relationKey;
      ...
      function rintExplanation(item, mode) {
        const task = {
          normal: "Normal switch compares the exact surface graph arrangement.",
          type: "Type switch compares the relation category only.",
          binary: "Binary switch compares whether the chain is logically valid.",
          rint: "RINT compares the derived relation state, not the surface tokens."
        }[mode] || "RINT compares the abstract relation state.";
        const nearMiss = item.isDistractor && item.nearMissReason ? ` Near miss: ${item.nearMissReason}.` : "";
        return `${task} Current: ${item.currentState}. N-back: ${item.backSummary}.${nearMiss}`;
      }

      function rintModeLabel(mode) {
        return {
          normal: "surface graph",
          type: "category",
          binary: "chain truth",
          rint: "derived relation"
        }[mode] || "relation state";
      }

      function rintEntityLabel(item) {
        return (item?.entities || item?.tokens || []).join(" / ");
      }

      function rintDerivedText(item) {
        if (!item) return "none";
        if (item.taskMode === "binary") return item.claim || (item.binaryTruth ? "valid chain" : "invalid chain");
        return item.derivedRelation?.text || item.query || item.premise || "derived relation";
      }
      ...
      function rintPremiseListHtml(item, compact = false) {
        const premises = item?.premises || [];
        if (!premises.length) return `<ol class="rint-premise-list"><li data-step="1">No premises yet.</li></ol>`;
        return `
          <ol class="rint-premise-list${compact ? " is-compact" : ""}">
            ${premises.map((premise, index) => `<li data-step="${index + 1}">${escapeHtml(premise.text)}</li>`).join("")}
          </ol>
        `;
      }

      function rintPromptHtml(item) {
        if (!item) {
          return `
            <div class="rint-prompt-shell">
              <span class="rint-prompt-kicker">premise graph</span>
              <p>Start Relational N-Back to generate an evolving relation stream.</p>
            </div>
          `;
        }
        return `
          <div class="rint-prompt-shell">
            <span class="rint-prompt-kicker">${escapeHtml(rintModeLabel(item.taskMode))} comparison</span>
            <div class="rint-prompt-grid">
              <div class="rint-premise-stack">
                <span class="rint-premise-label">current premises</span>
                ${rintPremiseListHtml(item)}
              </div>
              <div class="rint-derived-box">
                <span class="rint-derived-label">derive, then compare</span>
                <strong>${escapeHtml(rintDerivedText(item))}</strong>
                <span>${escapeHtml(rintRuleHint(item.taskMode))}</span>
              </div>
            </div>
          </div>
        `;
      ...
      function rintRuleHint(mode) {
        return {
          normal: "Match only if the surface graph arrangement equals the N-back graph.",
          type: "Match only if the relation category equals the N-back category.",
          binary: "Match only if claim truth equals the N-back claim truth.",
          rint: "Match only if the derived relation state equals the N-back derived state."
        }[mode] || "Match only if the active rule state equals the N-back state.";
      }

      function rintGraphChainHtml(item) {
        const entities = item?.entities || item?.tokens || [];
        const premises = item?.premises || [];
        if (!entities.length || !premises.length) {
          return `<div class="rint-graph-chain">${rintVisualToken("?", "node")}</div>`;
        }
        let html = rintVisualToken(entities[0], "A");
        premises.forEach((premise, index) => {
          html += `
            <div class="rint-graph-edge"><span>${escapeHtml(premise.relationLabel)}</span></div>
            ${rintVisualToken(entities[index + 1], String.fromCharCode(66 + index))}
          `;
        });
        return `<div class="rint-graph-chain">${html}</div>`;
      }
      ...
      function rintDiagramHtml(item, mode = item?.taskMode || "rint") {
        ...
        return `
          <div class="rint-diagram rint-diagram-${escapeHtml(family)}">
            <div class="rint-graph-canvas">
              ${rintGraphChainHtml(item)}
              <div class="rint-derived-readout">
                <small>${escapeHtml(mode === "type" ? rintCleanCategory(item.category) : "derived conclusion")}</small>
                <strong>${escapeHtml(rintDerivedText(item))}</strong>
              </div>
              <div class="rint-mini-premises">
                ${rintPremiseListHtml(item, true)}
              </div>
            </div>
          </div>
        `;
      ...
      function rintBinaryDiagramHtml(item) {
        ...
        return `
          <div class="rint-diagram rint-diagram-binary">
            <div class="rint-graph-canvas rint-binary-canvas">
              ${rintGraphChainHtml(item)}
              <div class="rint-binary-claim">
                <strong>${escapeHtml(status)}</strong>
                <span>${escapeHtml(item.claim || rintDerivedText(item))}</span>
              </div>
              <div class="rint-mini-premises">
                ${rintPremiseListHtml(item, true)}
              </div>
      ...
      function renderRintVisual() {
        ...
        const answerState = item && rint.locked
          ...
          : rintRuleHint(item?.taskMode || "rint");
        ...
        $("#rintCurrentCaption").textContent = item
          ? `${rintModeLabel(item.taskMode)}: ${rintDerivedText(item)}`
      ...
      function renderRintTrial() {
        ...
        const item = rint.current;
        $("#rintText").innerHTML = rintPromptHtml(item);
      ...
      function setupCanvas() {
        ...
        function draw(time = 0) {
          ...
          const now = time || performance.now();
          const deltaScale = lastCanvasTime ? clamp((now - lastCanvasTime) / BASE_FRAME_MS, 0.25, 2) : 1;
          lastCanvasTime = now;
          if (stageRect.width <= 1 || stageRect.height <= 1) {
            stageRect = $("#aimStage")?.getBoundingClientRect?.() || stageRect;
          }
          const rect = stageRect;
          ctx.clearRect(0, 0, rect.width, rect.height);
          particles.forEach((particle, index) => {
            const drift = (1 + moduleDifficulty("aim") * 0.12) * deltaScale;
      ...
      function bindEvents() {
        ...
        on($("#rintInstructionsButton"), "click", showRintInstructions, "rint instructions");
        on($("#rintInstructionsClose"), "click", closeRintInstructions, "close rint instructions");
        on($("#rintInstructionsStart"), "click", () => {
          closeRintInstructions();
          startRintStream();
        }, "start rint from instructions");
        $$("[data-close-rint-instructions]").forEach((button) => on(button, "click", closeRintInstructions, "close rint instructions"));
        ...
        let memoryTiltFrame = 0;
        let memoryTiltRect = null;
        let memoryTiltEvent = null;
        on($("#memoryGrid"), "pointermove", (event) => {
          if (state.settings.lowMotion) return;
          memoryTiltEvent = event;
          if (memoryTiltFrame) return;
          memoryTiltFrame = window.requestAnimationFrame(() => {
            memoryTiltFrame = 0;
            if (!memoryTiltEvent) return;
            memoryTiltRect = memoryTiltRect || $("#memoryGrid").getBoundingClientRect();
            const event = memoryTiltEvent;
            memoryTiltEvent = null;
            const rect = memoryTiltRect;
        ...
        on(window, "scroll", scheduleTopbarScrollState, "topbar scroll", { passive: true });
        if (window.PointerEvent) {
          on(document, "pointerdown", (event) => {
            armAudio();
            setPointerFromEvent(event);
            getCustomCursor()?.classList.add("is-pressing");
          }, "pointer down");
          on(document, "pointerup", (event) => {
            setPointerFromEvent(event);
            getCustomCursor()?.classList.remove("is-pressing");
          }, "pointer up");
          on(document, "pointermove", setPointerFromEvent, "cursor pointer move", { passive: true });
          on(document, "pointerover", setPointerFromEvent, "cursor pointer over", { passive: true });
          ...
          on(document, "mousemove", setPointerFromEvent, "cursor mouse move", { passive: true });
          on(document, "mouseover", setPointerFromEvent, "cursor mouse over", { passive: true });
        ...
        on(document, "keydown", (event) => {
          if (event.key === "Escape" && !$("#rintInstructionsModal")?.hidden) closeRintInstructions();
        }, "escape modal");
      }
