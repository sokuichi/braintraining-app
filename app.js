      ...
      function modeUnlocked(mode) {
        ...
        const current = order.indexOf(memoryUnlockForDifficulty(state.modules.memory.difficulty));
      ...
      function memoryConfig() {
        ...
        return {
          mode,
          modeDef,
          n: clamp(Math.ceil(d / 3) + (mode === "quad" ? 1 : 0), 1, 5),
          total: 10 + d * 2 + modeDef.channels.length,
          interval: clamp(2200 - d * 115 - modeDef.channels.length * 35, 780, 2200),
          grid: modeDef.grid,
      ...
      function renderMemoryStats() {
        ...
        $("#memoryTier").textContent = `T${clamp(Math.ceil(state.modules.memory.difficulty / 2), 1, 5)}`;
        ...
        const next = unlocks.find((item) => state.modules.memory.difficulty < item.at);
      ...
      function bindEvents() {
        ...
        $$("[data-memory-mode]").forEach((button) => button.addEventListener("click", () => setMemoryMode(button.dataset.memoryMode)));
        ...
        $("#memoryColorMatch").addEventListener("click", () => answerMemory("color"));
        $("#memoryTransformMatch").addEventListener("click", () => answerMemory("transform"));
        $("#memoryNoMatch").addEventListener("click", () => answerMemory("none"));
        $("#memorySubmit").addEventListener("click", submitMemoryAnswer);
        ...
        $("#quickSound").addEventListener("click", () => playSfx("level"));
